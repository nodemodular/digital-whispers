...
function setUserActive($token)
{
	...
	$sql = "UPDATE ".$db_table_prefix."Users SET Active = 1 WHERE ActivationToken ='".$db->sql_escape(trim($token))."' LIMIT 1";
...
function flagLostPasswordRequest($username,$value)
{
	...
	$sql = "UPDATE ".$db_table_prefix."Users SET LostPasswordRequest = '".$value."' WHERE Username_Clean ='".$db->sql_escape(sanitize($username))."' LIMIT 1";
...
function updateLastActivationRequest($new_activation_token,$username,$email)
{
	...
	$sql = "UPDATE ".$db_table_prefix."Users SET ActivationToken = '".$new_activation_token."', LastActivationRequest = '".time()."' WHERE Email = '".$db->sql_escape(sanitize($email))."' AND Username_Clean = '".$db->sql_escape(sanitize($username))."'";
